...
class Mo_Oauth_Widget extends WP_Widget {
	...
	public function mo_oauth_login_form() {
		...
		if( ! is_user_logged_in() ) {
			...
			if( isset($appsConfigured) && $appsConfigured ) {
				...
				if (is_array($appslist)) {
					foreach($appslist as $key=>$app){
						...
						// $temp .= '<a style="text-decoration:none" href="javascript:void(0)" onClick="moOAuthLoginNew(\''.$key.'\');">
						// Login with '.ucwords($key).'</a>';			
					}	
				}
			} else {
			...
			// $temp .= "
			// ".''."  <a style='text-decoration:none' href=\"".wp_logout_url( site_url() )."\" >Logout</a>
			// ";
